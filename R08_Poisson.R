...
# de 20 años.
# 
# Ejemplo 2. El número de personas que hacen cola delante de ti en el supermercado. Las variables 
...
(a <- exp(coef(poisson.model)[4])-1)
(b <- exp(coef(poisson.model)[2])-1)
(c <- exp(coef(poisson.model)[3])-1)
...
# Concluimos que el modelo se ajusta razonablemente bien porque
...
# con la desviación del modelo excluyendo esta variable cualitativa. 
...
# La prueba de chi-cuadrado de dos grados de libertad indica que programa, en conjunto,
# es una variable relevante para explicar el número de premios.


# A veces, es útil presentar los resultados de la regresión como índices de incidencia (es decir, 
# cambios porcentuales) y sus errores estándar, junto con el intervalo de confianza. Para calcular
# el error estándar de los cambios porcentuales, utilizaremos el método Delta del paquete "msm":
...
# negativo. El paquete R pscl proporciona muchas funciones para datos binomiales y de conteo, 
# incluyendo odTest para contrastar la sobredispersión.
...
# (8) La regresión de Poisson se estima por máxima verosimilitud que, por lo general, precisa de un 
