...
# Los modelos tobit, censurado y truncado son muy fáciles de confundir:
# Todos comparten situaciones donde no tenemos observaciones de una parte de la distribución normal,
# pero la situación puede ser muy diferente:
#   - Ejemplo 1: Algunas veces tenemos soluciones de esquina (corner solutions) en decisiones económicas:
#   mucha gente puede elegir consumir 0 unidades de un bien [esto sería un modelo de regresión Tobit]
#   - Ejemplo 2: Algunas veces la muestra se encuentra "capada" en una cantidad, por ejemplo, no tenemos
#   observaciones más alla del valor 40,001 o algo similar [esto sería un modelo de regresión censurado]
#   - Ejemplo 3: Puede ser que la variable dependiente sea el tiempo posterior a un tratamiento hasta
#   que los pacientes mueren, donde algunos pueden no haber muerto todavía [esto sería un modelo de
#                                                                           regresión censurado]
#   - Ejemplo 4: Un estadio de futbol se llena en un partido, lo cual significa que la demanda observada
#   se ve censurada en la capacidad del estadio [esto sería un modelo de regresión censurado]
#   - Ejemplo 5: Contamos con una muestra de observaciones con valores de la variable dependiente superiores
#   a un determinado límite [esto sería un modelo de regresión truncado]
