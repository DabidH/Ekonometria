...
# Las variables trabaja y I(horas>0) son iguales (dummy vs logica)
trab <- I(horas>0)
head(trab)
head(trabaja)
...
heckman.2s <- heckit(trabaja~hijosmen6 + hijosmay6 + edad + educ + ciudad,
...
probit.model <- glm(trabaja ~ hijosmen6 + hijosmay6 + edad + educ + ciudad, data = mroz,
...
(efm.heck <- mean ( coef(heckman)[10] + coef(heckman)[2]*rho.heck*s.heck*(((zb.heck/s.heck)^2)- (xb.heck*(zb.heck/s.heck))) ))
